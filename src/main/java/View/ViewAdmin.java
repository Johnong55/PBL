...
public class ViewAdmin extends JFrame {
	...
	public MyButton btnNewButton1, btnNewButton2, btnNewButton3_1, btnNewButton5, buttonAddClass, buttonDeleteClass,
	...
	public void ViewClass() {
		...
		table.setModel(
				new DefaultTableModel(new Object[][] {}, new String[] { "  Tên lớp", "  Số học sinh", "  Mã lớp" }));
	...
	public void ViewListSVinClass(String m) {
		...
		table.setModel(new DefaultTableModel(new Object[][] {},
				new String[] { "  Mã học sinh", "  Danh sách học sinh", "  Tên", " Mã lớp" }));
	...
	public void ViewAddSvInClass() {
		...
		DefaultTableModel model = new DefaultTableModel(new Object[][] {},
				new String[] { "  Danh sách học sinh chưa có lớp", "  Tên", "  Mã học sinh" });
		table1 = new MyTable();
	...
	public void ViewTeacher() {
		...
		DefaultTableModel model = new DefaultTableModel(new Object[][] {},
				new String[] { "  Mã giáo viên", "  Tên giáo viên", "  Tên" });
		table = new MyTable();
	...
	public void ViewClassOfTeacher(String m) {
		...
		table.setModel(
				new DefaultTableModel(new Object[][] {}, new String[] { "  Danh sách lớp", "  Mã Gv", "  Mã lớp" }));
	...
	public void ViewAddClassInGv() {
		...
		DefaultTableModel model = new DefaultTableModel(new Object[][] {},
				new String[] { "  Danh sách lớp", "  Số học sinh", "  Mã lớp" });
		table2 = new MyTable();
	...
	public void ViewStudent() {
		...
		DefaultTableModel model = new DefaultTableModel(new Object[][] {},
				new String[] { "  Mã học sinh", "  Tên học sinh", "  Lớp", "  Tên" });
		table = new MyTable();
		table.setModel(model);
		table.setRowHeight(30);
		table.setColor1(Color.WHITE);
	...
	public void ViewStudentJoinTest(String idclass, String idkithi) {
		...
		DefaultTableModel model = new DefaultTableModel(new Object[][] {},
				new String[] { "  Mã học sinh", "  Tên học sinh", "  Tên", "  Tên lớp" });
		table = new MyTable();
		table.setModel(model);
		table.setRowHeight(30);
		table.setColor1(Color.WHITE);
		...
		table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				if (e.getClickCount() == 2) {
					...
					System.out.println(idKiThi);
					if (b != null) {
						List<Cautraloisinhvien> cautraloi = CautraloiSinhvien_dao.Instance()
								.selectCautraloisinhvienfromBailam(b);
						List<Cauhoi> c = new ArrayList<Cauhoi>();
						String[] cA;
						cA = new String[cautraloi.size()];
						Arrays.fill(cA, "0");
						...
						JOptionPane.showMessageDialog(null, "Học sinh chưa làm bài", "Thông báo",
	...
	public void ViewTestOfStudent(String idSv) {
		...
		table.setModel(
				new DefaultTableModel(new Object[][] {}, new String[] { "  Lớp", "  Môn", "  Tên kì thi", "  Ngày thi",
						"  Thời gian bắt đầu", "  Thời gian thi", "  Số câu hỏi", "  Mã kì thi", "  Ma bai lam" }));
	...
	public String tendapAn(String q, Cauhoi c) {
		...
		if (c.getDapAnA().equals(q))
			k = "A";
		else if (c.getDapAnB().equals(q))
			k = "B";
		else if (c.getDapAnC().equals(q))
			k = "C";
		else if (c.getDapAnD().equals(q))
			k = "D";
		return k;
	}
	...
	public DefaultTableModel getModelExam() {
		...
		for (KiThi k : kthi) {
			Object[] row = { k.getLop().getTenlop(), k.getMonhoc(), k.getMota(), k.getDate().toString(),
					k.getThoigianbatdau().toString(), String.valueOf(k.getThoigianlambai()), String.valueOf(k.getSl()),
					k.getId() };
			model.addRow(row);
		}
		return model;
	...
	public DefaultTableModel getModelExamOfStudent(String idSv) {
		...
		for (KiThi k : kthi) {
			Object[] row = { k.getLop().getTenlop(), k.getNganhangcauhoi().getIdNganHang(), k.getMota(),
					k.getDate().toString(), k.getThoigianbatdau().toString(), String.valueOf(k.getThoigianlambai()),
					String.valueOf(k.getSl()), k.getId(), mabailam.get(i) };
			model.addRow(row);
			i++;
		}
		return model;
	...
	public DefaultTableModel getModelClasses(JTable table) {
		...
		for (Class c : listClass) {
			if (!c.getIdclass().equals("00")) {
				Object[] row = { c.getTenlop(), (c.getSvs() == null) ? "0" : String.valueOf(c.getSvs().size()),
						c.getIdclass() };
				model.addRow(row);
			}
		}
	...
	public void AddGv(String name, String id, String user, String pass) {
		...
		if (i == 1) {
			...
			JOptionPane.showMessageDialog(null, "Tạo giáo viên thành công", "Thông báo",
					JOptionPane.INFORMATION_MESSAGE);
		} else if (i == -1) {
			JOptionPane.showMessageDialog(null, "Mã giáo viên đã tồn tại", "Thông báo",
					JOptionPane.INFORMATION_MESSAGE);
		} else if (i == -2) {
			JOptionPane.showMessageDialog(null, "Tên đăng nhập đã tồn tại", "Thông báo",
					JOptionPane.INFORMATION_MESSAGE);
		}
	}
